...
def main():
    book_embeddings = np.load("../data/book_text_embeddings_bge.npz")
    books_file_path = "../data/extended_books_google.csv"
    books_data = pd.read_csv(books_file_path)
    books_data["full_text_embeddings"] = book_embeddings['embeddings'].tolist()
